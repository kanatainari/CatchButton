...
### C# 프로그래밍 학습
### 핵심 기능
1. 버튼위에 마우스를 올리면 버튼이 도망간다.
2. 해당 버튼을 잡으면(클릭하면) 축하 메시지와 함께 +10점이 된다.
3. 버튼은 잡을 수록 크기가 점점 작아지며 버튼을 잡지 못하면(배경을 클릭하면) -10점이 된다.
4. 기본 점수는 100점, 0점이 되면 게임오버 문구가 뜨며 재시작 여부를 묻는다. 
